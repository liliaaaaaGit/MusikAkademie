...
-- The error "column reference contract_id is ambiguous" occurs because both lessons and contracts tables have contract_id columns
...
BEGIN
  ...
  IF array_length_val IS NOT NULL THEN
    ...
    LOOP
      ...
      -- Update contract attendance count with explicit table references
      UPDATE contracts c
      SET 
        attendance_count = (
          SELECT 
