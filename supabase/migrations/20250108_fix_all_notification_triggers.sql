-- Comprehensive fix for all notification triggers that reference students.teacher_id
-- This migration fixes the "column s.teacher_id does not exist" error when completing contracts
...
BEGIN
  ...
  IF should_notify THEN
    ...
    SELECT 
      s.name,
      s.id,
      COALESCE(t.name, 'Unbekannter Lehrer') as teacher_name,
      NEW.teacher_id as teacher_id_val
    INTO 
      student_name,
      student_id_val,
      teacher_name,
      teacher_id_val
    FROM students s
    LEFT JOIN teachers t ON t.id = NEW.teacher_id  -- FIXED: Use contracts.teacher_id
