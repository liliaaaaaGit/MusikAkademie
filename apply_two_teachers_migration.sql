-- Two Teachers Per Student Feature Migration
...
begin
  ...
  if not exists (select 1 from pg_policies where polname = 'student_teachers_admin_all' and tablename = 'student_teachers') then
  ...
  if not exists (select 1 from pg_policies where polname = 'student_teachers_teacher_read' and tablename = 'student_teachers') then
  ...
  if not exists (select 1 from pg_policies where polname = 'student_teachers_teacher_insert' and tablename = 'student_teachers') then
  ...
  if not exists (select 1 from pg_policies where polname = 'student_teachers_teacher_delete' and tablename = 'student_teachers') then
...
create or replace function public.is_teacher_of_student(_student bigint)
returns boolean
language sql stable security definer set search_path=public as $$
  select exists(
