-- Simple fix for ambiguous contract_id reference in atomic_save_and_sync_contract function
-- This updates only the problematic UPDATE statement

BEGIN;

-- Drop and recreate the function to ensure we have the latest version
DROP FUNCTION IF EXISTS public.atomic_save_and_sync_contract(jsonb, boolean, uuid, uuid);
