-- Direct fix for ambiguous contract_id reference
-- This only fixes the specific problematic line without recreating the entire function

BEGIN;

-- First, let's see what the current function looks like
-- Then we'll create a minimal fix

-- Create a simple test to see if the function exists and what it looks like
DO $$
DECLARE
    func_exists boolean;
BEGIN
    ...
    IF func_exists THEN
        RAISE NOTICE 'Function atomic_save_and_sync_contract exists';
    ELSE
        RAISE NOTICE 'Function atomic_save_and_sync_contract does NOT exist';
    END IF;
END $$;

-- Now let's create a completely new, clean version of the function
-- This ensures we have the latest version with all fixes
...
BEGIN
    -- Generate lock key
    lock_key := CASE 
        WHEN is_update THEN hashtext(contract_id_param::text)
        ELSE hashtext('new_contract_' || pg_backend_pid()::text) 
    END;
    ...
    -- Handle discount_ids - check if it's a JSON array
    ...
    IF is_update THEN
        ...
        SET
            ...
            billing_cycle = CASE 
                WHEN contract_data->>'billing_cycle' IS NOT NULL 
                THEN (contract_data->>'billing_cycle')::billing_cycle 
                ELSE NULL 
            END,
            paid_at = CASE 
                WHEN contract_data->>'paid_at' IS NOT NULL 
                THEN (contract_data->>'paid_at')::timestamptz 
                ELSE NULL 
            END,
            paid_through = CASE 
            ...
            END,
            term_end = CASE 
                WHEN contract_data->>'term_end' IS NOT NULL 
                THEN (contract_data->>'term_end')::timestamptz 
                ELSE NULL 
            END,
            term_label = contract_data->>'term_label',
            cancelled_at = CASE 
                WHEN contract_data->>'cancelled_at' IS NOT NULL 
                THEN (contract_data->>'cancelled_at')::timestamptz 
                ELSE NULL 
            END,
            updated_at = now()
    ...
    -- FIXED: Update operation log with explicit table reference
...
EXCEPTION
    WHEN OTHERS THEN
        -- FIXED: Log error with explicit table reference
        UPDATE public.contract_operation_log
        SET 
            status = 'failed', 
